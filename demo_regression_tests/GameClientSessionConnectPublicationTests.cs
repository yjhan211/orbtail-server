    ...
    private sealed class ConnectFixture : IDisposable
    {
            ...
            Action<GameClientSession>? recordEntryFailure = null)
        {
            ...
            var session = new GameClientSession(
                ...
                new InGameInventoryManager(Store.Get),
                new GroundItemManager(Store.Get),
                new SummonStoneManager(Store.Get),
                new MatchRosterManager(Store.Get, NullLogger.Instance),
                new AreaClosureManager(Store.Get, NullLogger.Instance),
                ...
                new EncounterRevealManager(Store.Get),
                Store,
                static (_, _, _, _) => { },
                static (_, _, _, _, _) => { },
