...
public sealed class MovementPacketQueueTests
{
    ...
    public async Task GameSessionPacketEntry_KeepsFinalStopBeforeAction_AndRejectsInvalidReplacement()
    {
        ...
        Task Move(float position) => Send(session, Protocol.C_TO_G_MOVE,
            new C_TO_G_MOVE { Position = new Vector3f(position, 0, 0), Velocity = new Vector3f() });

        await Move(1);
        Task pending = Move(2);
        await clock.NextDelayAsync();
        Task stop = Move(3);
        await Move(float.NaN); // 정상 정지 요청을 잘못된 위치로 덮어쓰지 않는다.
    ...
    public async Task FinalStopIsAppliedWithoutAnotherPacket_AndBurstKeepsOnlyLatestPosition()
    {
        ...
        Task Send(int position, int velocity) => queue.EnqueueAsync(() =>
        {
        ...
        Task pending = Send(11, 6);
        Assert.Equal(TimeSpan.FromMilliseconds(9), await clock.NextDelayAsync());
        Task newer = Send(12, 6);
        // 마지막에 받은 정지 통지가 대기 중인 이동을 대체한다.
        Task stop = Send(13, 0);
    ...
    public async Task MovementCannotBeCoalescedAcrossAnActionPacket()
    {
        ...
        Task Send(string name, bool isMovement) => queue.EnqueueAsync(() =>
        {
            calls.Add(name);
            return Task.CompletedTask;
        }, isMovement);

        await Send("move1", true);
        Task stop = Send("stop", true);
        await clock.NextDelayAsync();
        Task action = Send("explore", false);
        Task nextMove = Send("move3", true);
        Task latest = Send("move4", true);
    ...
    [Fact]
    public async Task LastReceivedMovementWinsRegardlessOfPayloadValue()
    {
        var clock = new ManualClock();
        var queue = new MovementPacketQueue(() => true, clock);
        var values = new List<uint>();
        Task Send(uint value) => queue.EnqueueAsync(() =>
        {
            values.Add(value);
            return Task.CompletedTask;
        }, true);
        ...
        await clock.NextDelayAsync();
        Task next = Send(0);
        Task last = Send(uint.MaxValue - 2);
        Assert.Same(pending, next);
        Assert.Same(next, last);
        clock.Advance(TimeSpan.FromMilliseconds(10));
        await last.WaitAsync(TestTimeout);
        Assert.Equal(new[] { uint.MaxValue - 1, uint.MaxValue - 2 }, values);
    }
    ...
    public async Task DisconnectSkipsPendingMoveAndAction_ButCompletesTheirTasks()
    {
        ...
        await queue.EnqueueAsync(Dispatch, true);
        Task stop = queue.EnqueueAsync(Dispatch, true);
        await clock.NextDelayAsync();
    ...
    public async Task IntervalStartsAfterHandlerCompletes_AndEarlyTimerDoesNotProcessTooSoon()
    {
        ...
        Task first = queue.EnqueueAsync(() => release.Task, true);
        bool processed = false;
        Task stop = queue.EnqueueAsync(() => { processed = true; return Task.CompletedTask; }, true);
    ...
    public async Task HandlerFailureIsObservedAndDoesNotStrandFollowingRequests()
    {
        var clock = new ManualClock();
        var queue = new MovementPacketQueue(() => true, clock);
        await queue.EnqueueAsync(() => Task.CompletedTask, true);
        Task failure = queue.EnqueueAsync(() => Task.FromException(new InvalidOperationException("test")), true);
        await clock.NextDelayAsync();
        bool actionProcessed = false;
