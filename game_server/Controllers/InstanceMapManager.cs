    ...
    : BaseMapManager(logger, natsClient, cacheHelper, serverConfig)
{
    ...
    private void StartGameTimer(string instanceKey, long mapSubId)
    {
        ...
        var gameTimer = new Timer(
            _ => RunTrackedHandler(async () =>
            {
                try
                {
                    await EndGameAsync(instanceKey, mapSubId);
                }
                finally
                {
                    if (_gameTimers.TryRemove(instanceKey, out var timer))
                        timer.Dispose();
    ...
    private async Task EndGameAsync(string instanceKey, long mapSubId)
    {
        if (IsStopping)
            return;
    ...
    /// </summary>
    private void CleanupInstance(string instanceKey)
    {
        // 게임 타이머 정리
    ...
    {
        var gameTimers = new List<Timer>();
        var warningTimers = new List<Timer>();
        ...
        {
            Logger.LogWarning(ex, "InstanceMapManager NATS subscription shutdown failed");
        }

        await MapLock.WaitAsync();
        try
        {
            // Detach while holding MapLock so EnterInstance cannot publish new timers
            // into the dictionaries during shutdown. Awaiting callbacks while holding
            // this lock deadlocks when EndGame is already waiting for the same lock.
            foreach (string instanceKey in _gameTimers.Keys)
            {
                if (_gameTimers.TryRemove(instanceKey, out var timer))
                    gameTimers.Add(timer);
            }

            foreach (string instanceKey in _warningTimers.Keys)
            {
                if (!_warningTimers.TryRemove(instanceKey, out var timers))
                    continue;
                warningTimers.Add(timers.OneMinuteTimer);
                warningTimers.Add(timers.ThirtySecondsTimer);
            }
        }
        finally
        {
        ...
        foreach (var timer in warningTimers)
            await timer.DisposeAsync();
