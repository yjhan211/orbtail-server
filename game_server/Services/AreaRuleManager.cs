...
namespace game_server.services
{
    ...
    public class MatchingAreaRuleState
    {
        ...
        /// 적용된 모든 규칙 ID 목록
        /// </summary>
        public List<int> AllRuleIds => new List<int>(_allRuleIds);
    }
    ...
    public class AreaRuleManager
    {
        ...
        public MatchingAreaRuleState GetOrCreateMatchingState(long matchingId, Random? random = null)
        {
            return _matchingStates.GetOrAdd(matchingId, id =>
            {
                ...
                _logAction?.Invoke($"AreaRuleManager: Created rule state for MatchingId={id}, Rules=[{string.Join(", ", state.AllRuleIds)}]");
                return state;
            });
        }
        ...
        public Dictionary<AreaType, List<int>> GetAllRules(long matchingId)
        {
            ...
            foreach (var ruleId in state.AllRuleIds)
            {
                ...
                if (ruleData != null)
                {
                    if (!result.ContainsKey(ruleData.AreaType))
                    {
                        result[ruleData.AreaType] = new List<int>();
                    }
        ...
        public int GetRuleForNote(long matchingId)
        {
            ...
            _logAction?.Invoke($"AreaRuleManager: MatchingId={matchingId} note used, RuleId={ruleId}, remaining={state.RemainingRuleCount}");
            return ruleId;
        }
