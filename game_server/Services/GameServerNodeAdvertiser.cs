...
///     종료 시작 시 accepting=false로 바꿔 새 배정을 막은 뒤 정리가 끝나면 항목을 지운다.
    ...
    ILogger logger) : IAsyncDisposable
{
    ...
    /// <summary>새 배정을 막는다. 진행 중인 매치는 계속 돈다.</summary>
    public async Task StopAcceptingAsync()
    {
        ...
        {
            logger.LogWarning(ex, "Game server node could not publish draining state: NodeId={NodeId}", NodeId);
        }
    }
