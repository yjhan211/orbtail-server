...
/// </summary>
public class MatchingPoolState(long matchingId)
{
    ...
    private void EnsurePoolInitialized(int interactId, int poolId)
    {
        ...
        if (originalPool.Count <= 0) return;
        // interactId와 matchingId를 조합한 시드로 셔플
        var random = new Random((int)((matchingId * 1000000L + interactId) % int.MaxValue));
    ...
    public int? GetNextItem(int interactId, int poolId)
    {
        lock (_poolLock)
        {
            ...
            if (!_poolIndices.TryGetValue(key, out int index))
                return null;
...
public class ItemPoolManager
{
    ...
    /// </summary>
    private MatchingPoolState GetOrCreatePoolState(long matchingId)
    {
    ...
    public int? GetNextItemFromPool(long matchingId, int interactId, int poolId)
    {
        ...
        int? item = state.GetNextItem(interactId, poolId);

        _logAction?.Invoke(
            item.HasValue
                ? $"ItemPoolManager: MatchingId={matchingId} InteractId={interactId} got item {item.Value} from pool {poolId}, remaining={state.GetRemainingCount(interactId, poolId)}"
                : $"ItemPoolManager: MatchingId={matchingId} InteractId={interactId} pool {poolId} exhausted or not found");
