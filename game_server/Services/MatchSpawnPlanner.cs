...
///     DEV_CROSSFIRE_SANDBOX=1이면 전원 운동장 스폰(교차사격 실험용 — 정식 흐름은 잠긴 시작방에서 운동장까지 100초가 걸린다).
/// </summary>
internal static class MatchSpawnPlanner
{
    public static readonly bool IsCrossfireSandbox =
        Environment.GetEnvironmentVariable("DEV_CROSSFIRE_SANDBOX") == "1";

    public static IReadOnlyDictionary<long, Cell> Plan(long matchingId, MapId mapId, IEnumerable<long> playerIds)
    {
        var ids = playerIds.Distinct().ToList();
        if (!IsCrossfireSandbox)
