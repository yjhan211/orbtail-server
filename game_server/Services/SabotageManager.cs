...
namespace game_server.services
{
    ...
    {
        public AreaType TriggerArea { get; set; }  // 이 Area에 진입하면 트리거
        public int InteractId { get; set; }        // 상태 변경할 Interactable ID
        public int DelaySeconds { get; set; }      // 트리거 후 지연 시간 (초)
        public int NewState { get; set; }          // 변경할 상태 값

        // 타임아웃 설정
        public int TimeoutSeconds { get; set; }    // 상태 변경 후 타임아웃 시간 (초), 0이면 타임아웃 없음
        public int TimeoutState { get; set; }      // 타임아웃 시 변경할 상태
        public int TimeoutCorruption { get; set; } // 타임아웃 시 모든 플레이어 정신오염도 증가량

        // 해결 조건 (액션 완료 시 타임아웃 취소)
        public int ResolveActionId { get; set; }   // 이 ActionId 완료 시 타임아웃 타이머 취소, 0이면 없음
    }

    /// <summary>
    /// 매칭별 사보타주 상태 관리
    /// </summary>
    public class MatchingSabotageState
    {
        ...
        public MatchingSabotageState(long matchingId)
        {
        ...
        {
            if (_activeTimers.TryRemove(interactId, out var timer))
            {
                timer.Dispose();
            }
        }
        ...
        {
            if (_timeoutTimers.TryRemove(interactId, out var timer))
            {
                timer.Dispose();
            }
        ...
        {
            return _activeEvents.TryGetValue(interactId, out var evt) ? evt : null;
        }
    ...
    public class SabotageManager
    {
        ...
        public void OnPlayerEnterArea(long matchingId, AreaType areaType)
        {
            ...
            if (!_sabotageEvents.TryGetValue(areaType, out var events))
            {
                return;
            }
            ...
            _logAction?.Invoke($"SabotageManager: Area {areaType} triggered for MatchingId={matchingId}, starting {events.Count} sabotage events");

            // 각 사보타주 이벤트에 대해 타이머 시작
            foreach (var evt in events)
            {
                StartSabotageTimer(matchingId, evt);
            }
        }
        ...
        private void StartSabotageTimer(long matchingId, SabotageEvent evt)
        {
            ...
            _logAction?.Invoke($"SabotageManager: Starting timer for InteractId={evt.InteractId}, delay={evt.DelaySeconds}s, newState={evt.NewState}");

            var timer = new Timer(_ =>
            {
                OnSabotageTimerElapsed(matchingId, evt);
            }, null, TimeSpan.FromSeconds(evt.DelaySeconds), Timeout.InfiniteTimeSpan);
        ...
        {
            _logAction?.Invoke($"SabotageManager: Timer elapsed for MatchingId={matchingId}, InteractId={evt.InteractId}, setting state to {evt.NewState}");

            // 시작 타이머 정리
            if (_matchingStates.TryGetValue(matchingId, out var matchingState))
            {
                matchingState.RemoveTimer(evt.InteractId);
            }
            ...
            {
                StartTimeoutTimer(matchingId, evt);
            }
        }
        ...
        private void StartTimeoutTimer(long matchingId, SabotageEvent evt)
        {
            ...
            _logAction?.Invoke($"SabotageManager: Starting timeout timer for InteractId={evt.InteractId}, timeout={evt.TimeoutSeconds}s");

            var timer = new Timer(_ =>
            {
                OnTimeoutTimerElapsed(matchingId, evt);
            }, null, TimeSpan.FromSeconds(evt.TimeoutSeconds), Timeout.InfiniteTimeSpan);
        ...
        {
            _logAction?.Invoke($"SabotageManager: TIMEOUT for MatchingId={matchingId}, InteractId={evt.InteractId}, applying corruption +{evt.TimeoutCorruption}");
        ...
        public void OnActionCompleted(long matchingId, int interactId, int actionId)
        {
            ...
            {
                _logAction?.Invoke($"SabotageManager: Sabotage resolved for MatchingId={matchingId}, InteractId={interactId}, ActionId={actionId}");
        ...
        public void Reset()
        {
            foreach (var state in _matchingStates.Values)
            {
                state.Cleanup();
            }
