namespace network.common
{
    ...
    public static class GameDesignData
    {
        ...
        )
        {
            ...
            {
                conditions.Add(202000015, new() { (202000011, 1), (202000012, 1) }); // 연구원 정식 (김치 참치 볶음밥 + 소고기 라면)
                conditions.Add(202000016, new() { (202000013, 1), (202000014, 1) }); // 굿 밸런스 프로틴 (프리 타임 + 비타천)
            }
            if (researches.Any(r => r.id == 2 && r.level >= 2))
            {
                conditions.Add(202000017, new() { (202000015, 1), (202000016, 1), (301000055, 1) }); // 나이트 워커 밀 (연구원 정식 + 굿 밸런스 프로틴 + 카페인 추출물)
                conditions.Add(202000018, new() { (202000014, 1), (301000056, 1), (301000060, 1) }); // 쿨파이브 (비타천 + 타우린 파우더 + 과일 통조림)
            }

            // 분석 화학 (ID: 3)
            if (researches.Any(r => r.id == 3 && r.level >= 1))
            {
                conditions.Add(202000007, new() { (301000058, 1), (301000063, 1) }); // 참치 볶음 (참치 통조림 + 조미료 팩)
                conditions.Add(202000008, new() { (301000061, 1), (301000062, 1) }); // 수상한 라면 (인스턴스 면 + 건조 야채)
            }
            if (researches.Any(r => r.id == 3 && r.level >= 2))
            {
                conditions.Add(202000009, new() { (301000060, 1), (301000065, 1) }); // 탕후루 (과일 통조림 + 결정제 설탕)
                conditions.Add(202000010, new() { (301000067, 1), (301000068, 1) }); // 밸런스 프로틴 (압축 단백질 + 견과류 조각)
            }
            ...
            {
                conditions.Add(401000002, new() { (302000003, 1), (302000004, 1), (302000006, 1) }); // 허름한 좌판 (전도성 섬유 + 내화 섬유 + 방탄 섬유)
            }
            ...
            if (researches.Any(r => r.id == 5 && r.level >= 2))
            {
                ...
                conditions.Add(104000004, new() { (104000001, 1), (302000005, 1), (302000006, 1) }); // 습지 탐사 장화 (연구원의 구두 + 충격 흡수 패드 + 방탄 섬유)
            }

            // 유기 화학 (ID: 6)
            if (researches.Any(r => r.id == 6 && r.level >= 1))
            {
                conditions.Add(202000011, new() { (202000007, 1), (301000072, 1) }); // 김치 참치 볶음밥 (참치 볶음 + 김치)
                conditions.Add(202000012, new() { (202000008, 1), (301000077, 1) }); // 소고기 라면 (수상한 라면 + 소고기 육포)
            }
            if (researches.Any(r => r.id == 6 && r.level >= 2))
            {
                conditions.Add(202000013, new() { (202000009, 1), (301000070, 1), (301000071, 1) }); // 프리 타임 (탕후루 + 초콜릿 조각 + 카카오 버터)
                conditions.Add(202000014, new() { (202000010, 1), (301000069, 1), (301000057, 1) }); // 비타천 (밸런스 프로틴 + 건조 과일 조각 + 비타민 복합체)
            }
