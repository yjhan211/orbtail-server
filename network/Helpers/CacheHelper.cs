    ...
    public class CacheHelper
    {
        ...
        public static void Initialize(RedisConnectionPool redisPool)
        {
            lock (_lock)
            {
                if (_instance != null)
                {
                    throw new InvalidOperationException("CacheHelper is already initialized.");
                }
        ...
        public static CacheHelper Instance
        {
            get
            {
                if (_instance == null)
                {
                    throw new InvalidOperationException("[CacheHelper] CacheHelper is not initialized");
                }
        ...
        public async Task<T> ExecuteRedisCommandAsync<T>(Func<IDatabase, Task<T>> action, int db = -1)
        {
        ...
        {
            return ExecuteRedisCommandAsync(database => database.HashExistsAsync(key, field, CommandFlags.PreferReplica), db);
        }
        ...
        {
            return ExecuteRedisCommandAsync(database => database.ListRangeAsync(key, start, end, CommandFlags.PreferReplica), db);
        }
        ...
        public async Task<List<(string key, RedisValue value)>> ListRangeWithKeyAsync(List<string> keys, int start = 0, int end = -1, int db = -1)
        {
            if (keys == null || keys.Count == 0)
            {
        ...
        {
            RedisValue result = await ExecuteRedisCommandAsync(database => database.ListLeftPopAsync(key), db);
