...
namespace game_server
{
    ...
    public class PacketQueueProcessor
    {
        ...
        public PacketQueueProcessor(Player player)
        {
            packet_queue = new();
                ...
                try
                {
                    lock (loop_lock)
                    {
                        List<GameObject> game_object_list =
                            Program.game_server.GetBoundGameObjectList(this.player.target_cell);

                        // 오브젝트 최대한 모아서 보내기
                        // if (game_object_list.Count > 0)
                        // {
                        //     Packet packet = GameServer.MakeMapInfoObject(player_obj_list);
                        //     player.Send(packet);
                        // }
                    }
