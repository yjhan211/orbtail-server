...
namespace user_server.controllers
{
    public static class JobController
    {
        public static async Task GetExploreTargetInfo(GameUser user, C_TO_U_EXPLORE_TARGET_INFO body)
        {
            ...
            for (int i = 0; i < body.ExploreTargetIdList.Count; i++)
            {
                var targetExploreUid = body.ExploreTargetIdList[i];
                ExploreTargetInfo? targetExploreInfo;

                using (await ExploreTargetInfo.Lock(user.RedLock, targetExploreUid))
                {
                ...
                bool isMax = targetInfoList.Count >= Config.BROADCAST_UNIT;
                bool isEnded = (i == targetInfoList.Count - 1) || (targetInfoList.Count == targetInfoList.Count);
        ...
        public static async Task GetJobResourceInfo(GameUser user, C_TO_U_JOB_RESOURCE_INFO body)
        {
            ...
            for (int i = 0; i < body.JobResourceIdList.Count; i++)
            {
                var targetResourceId = body.JobResourceIdList[i];
                JobResourceInfo? targetResourceInfo;

                using (await JobResourceInfo.Lock(user.RedLock, targetResourceId))
                {
        ...
        public static async Task UpgradeJob(GameUser user, C_TO_U_UPGRADE_JOB body)
        {
            ...
            var maxExp = GameDesignData.GetMaxExp(jobStat.JobGrade);
            ...
            JobGrade targetGrade = jobStat.JobGrade + 1;
        ...
        public static List<int> GetSkillList(PlayerInfo playerInfo)
        {
            ...
            {
                var (_, skillId, _) = GameDesignData.GetSkill(wearItem);
                if (skillId <= 0)
                {
                    continue;
                }
        ...
        public static async Task Explore(GameUser user, C_TO_U_EXPLORE body)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                using (await ExploreTargetInfo.Lock(user.RedLock, body.ExploreTargetUid))
                {
                    ...
                    var exploreTargetDetail = GameDesignData.GetExploreTargetDetail(
                    ...
                    var (isValidJobType, isExploreAble, isEnoughLevel) =
                        playerInfo.WearItemIdList.Aggregate(
                            ...
                            (acc, wearItem) =>
                            {
                                var (skillType, skillId, skillLevel) = GameDesignData.GetSkill(wearItem);
                                return (
        ...
        public static async Task ExploreEnd(GameUser user, ExploreTargetInfo exploreTargetInfo)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                bool isSuccess = random.Next(0, 100) < 50;
                if (isSuccess)
                {
                    ...
                    var upgradePool = GameDesignData.GetExploreResultPool(exploreTargetInfo.ExploreTargetId);
        ...
        public static async Task UseJobSkill(GameUser user, C_TO_U_USE_SKILL body)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                var skillDetail = GameDesignData.GetSkillDetail(body.SkillId);
                using (await JobResourceInfo.Lock(user.RedLock, body.ResourceUid))
                {
                    ...
                    var jobResourceDetail = GameDesignData.GetJobResourceDetail(jobResourceInfo.ResourceId);
        ...
        public static async Task Encamp(GameUser user, C_TO_U_ENCAMP body)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                user.SetState(playerInfo, PlayerState.CAMIPING_1);
        ...
        public static async Task AddSellItem(GameUser user, C_TO_U_ADD_SELL_ITEM body)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                if (!playerInfo.InventoryInfo.ItemDict.TryGetValue(body.ItemUid, out ItemInfo? itemInfo))
                {
                    return;
                }

                if (3 <= campInfo.CellDict.Count)
                {
                    return;
                }

                if (itemInfo.IsWear || itemInfo.Count <= 0)
                {
                    return;
                }
        ...
        public static async Task BuyItem(GameUser user, C_TO_U_BUY_ITEM body)
        {
            ...
            using (await PlayerInfo.Lock(user.RedLock, user.PlayerId))
            {
                ...
                if (!campInfo.CellDict.TryGetValue(body.SellItemUid, out var sellItemInfo))
                {
                    return;
                }
                ...
                using (await PlayerInfo.Lock(user.RedLock, body.SellerId))
                {
                    var item_info = sellItemInfo.Item1;
                    if (item_info == null)
                    {
                        return;
                    }
